...
2. Uprav lokálne.
3. **Sanity brány — všetky tri musia prejsť:**
...
4. **Každý `str_replace` a textová náhrada musí byť overená.** Náhrada, ktorá
   nenájde kotvu, prebehne **naprázdno a bez chyby** — vždy over, že zmena
   naozaj pristála (assert alebo spätná kontrola).
5. Commitni cez GitHub contents API (GET sha → PUT base64 + sha).
6. Verzia `vNN` v changelogu; správa commitu vecne popisuje zmenu a odkazuje na normatívne ID.
